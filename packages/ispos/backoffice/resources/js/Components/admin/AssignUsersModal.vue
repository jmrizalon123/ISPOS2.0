...
const gridOptions = computed(() => ({
    ...
    overlayNoRowsTemplate: `<span class="assign-users-grid-empty">${t('storeEmployees.noUsersMatch')}</span>`,
}));
...
function submit() {
    form.post(route('admin.users.store-employees.bulk-store'), {
        preserveScroll: true,
    ...
    <Modal :show="show" max-width="lg" @close="emit('close')">
            ...
            <div class="flex min-w-0 flex-1 flex-wrap items-start justify-between gap-3 pr-6">
                <div class="min-w-0">
                    <h3 class="font-display text-lg font-semibold text-ink">
                        {{ t('storeEmployees.assignModalTitle') }}
                    </h3>
                    <p class="mt-1 text-sm leading-relaxed text-ink-muted">
                        {{ t('storeEmployees.assignModalDescription', { store: storeName }) }}
                    </p>
                </div>
                <div class="flex shrink-0 flex-wrap items-center gap-2">
                    ...
                    <Button type="button" :disabled="form.processing || form.user_ids.length === 0" @click="submit">
                        {{ t('storeEmployees.assignSelected', { count: form.user_ids.length }) }}
                    </Button>
                </div>
            </div>
        </template>
        ...
        <div class="mb-3 flex flex-wrap items-center gap-2">
            ...
            <Badge variant="accent" class="shrink-0 tabular-nums">
                {{ t('storeEmployees.selectedCount', { count: form.user_ids.length }) }}
            </Badge>
        </div>
        ...
        <p v-else class="mt-3 rounded-xl border border-line bg-surface-muted/20 px-4 py-10 text-center text-sm text-ink-muted">
            {{ users.length ? t('storeEmployees.noUsersMatch') : t('storeEmployees.noAvailableUsers') }}
        </p>
