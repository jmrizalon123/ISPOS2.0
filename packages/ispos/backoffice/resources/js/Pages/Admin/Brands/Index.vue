...
const page = useModulePage('brands');
const { t } = useI18n();
...
const countLabel = useRecordCountLabel(() => props.brands.total, 'brand');

const columns = computed(() => [
    lineCol.value,
    { key: 'brand_code', label: t('common.code') },
...
<template>
    ...
    <AppLayout>
        ...
        <div class="index-page flex flex-col gap-2" :class="brands.data.length ? 'index-page--fill' : undefined">
            <IndexPageHeader
                class="shrink-0"
            ...
            >
                ...
                <template v-if="can('brands.create')" #actions>
        ...
        <EmptyState
            ...
            :action-label="can('brands.create') ? emptyAction('createBrand') : undefined"
            @action="can('brands.create') && router.visit(route('admin.brands.create'))"
        />

        <DataTable v-else :columns="columns" :rows="brands.data" :paginated="brands" pagination-route="admin.brands.index" :pagination-query="filterQuery" sticky-actions viewport-fit compact>
            <template #cell-line="{ index }">
                <span class="tabular-nums text-ink-muted">{{ paginatedRowNumber(brands.from, index) }}</span>
            </template>
            <template #cell-name="{ row }">
                <Link
                    :href="route('admin.brands.edit', (row as BrandRecord).id)"
                    class="font-medium text-accent hover:underline"
                >
                    {{ (row as BrandRecord).name }}
                </Link>
            </template>
            <template #cell-company="{ row }">
                {{ (row as BrandRecord).company?.name ?? '—' }}
            </template>
            <template #cell-status="{ row }">
                <Badge :variant="(row as BrandRecord).status === 'active' ? 'success' : 'neutral'">
                    {{ (row as BrandRecord).status }}
                </Badge>
            </template>
            <template #cell-creator="{ row }">
                <TableAuditStamp :user="(row as BrandRecord).creator" :at="(row as BrandRecord).created_at" />
            </template>
            <template #cell-last_modifier="{ row }">
                <TableAuditStamp :user="(row as BrandRecord).updater" :at="(row as BrandRecord).updated_at" />
            </template>
            <template #actions="{ row }">
                <Link :href="route('admin.brands.edit', row.id)" class="text-sm text-accent hover:underline">{{ t('common.edit') }}</Link>
            </template>
        </DataTable>
        </div>
    </AppLayout>
