...
import { defaultBrandForm, type BrandFormData, type BrandRecord } from '@/types/brand';
...
const pageTitle = useFormPageTitle('brand', isEdit);

const form = useForm<BrandFormData>(defaultBrandForm(props.brand, props.companies[0]?.id ?? ''));
...
<template>
    ...
    <AppLayout>
        ...
        <FormShell
            ...
            :back-href="route('admin.brands.index')"
        >
            <form class="form-card-form" @submit.prevent="submitForm">
                <div class="form-card-scroll">
                    <FormSection title-key="brandDetails" description-key="brandDetails">
                        <div class="form-grid">
                            ...
                            <FormField label-key="code" required :error="form.errors.brand_code">
                                <Input v-model="form.brand_code" />
                            </FormField>
                ...
                <FormActionBar
                    :cancel-href="route('admin.brands.index')"
                    :submit-label="isEdit ? submit('saveBrand') : submit('createBrand')"
