...
import type { PriceGroupRecord } from '@/types/priceGroup';
...
const extraCols = useColumns([{ key: 'is_default', col: 'default' }]);
const lineCol = useLineTableColumn();

const props = defineProps<{
    priceGroups: Paginated<PriceGroupRecord>;
...
const countLabel = useRecordCountLabel(() => props.priceGroups.total, 'priceGroup');

const columns = computed(() => [
    lineCol.value,
    { key: 'price_group_code', label: t('common.code') },
...
<template>
    ...
    <AppLayout>
        ...
        <div class="index-page flex flex-col gap-2" :class="priceGroups.data.length ? 'index-page--fill' : undefined">
            <IndexPageHeader
                class="shrink-0"
            ...
            >
                ...
                <template v-if="can('price_groups.create')" #actions>
                    <Link :href="route('admin.price-groups.create')">
        ...
        <EmptyState
            v-if="!priceGroups.data.length"
            :title="page.emptyTitle"
            :description="page.emptyDescription"
            :action-label="can('price_groups.create') ? emptyAction('createPriceGroup') : undefined"
            @action="can('price_groups.create') && router.visit(route('admin.price-groups.create'))"
        />

        <DataTable v-else :columns="columns" :rows="priceGroups.data" :paginated="priceGroups" pagination-route="admin.price-groups.index" :pagination-query="filterQuery" sticky-actions viewport-fit compact>
            <template #cell-line="{ index }">
                <span class="tabular-nums text-ink-muted">{{ paginatedRowNumber(priceGroups.from, index) }}</span>
            </template>
            <template #cell-name="{ row }">
                <Link
                    :href="route('admin.price-groups.edit', (row as PriceGroupRecord).id)"
                    class="font-medium text-accent hover:underline"
                >
                    {{ (row as PriceGroupRecord).name }}
                </Link>
            </template>
            <template #cell-is_default="{ row }">
                {{ (row as PriceGroupRecord).is_default ? 'Yes' : '—' }}
            </template>
            <template #cell-company="{ row }">
                {{ (row as PriceGroupRecord).company?.name ?? '—' }}
            </template>
            <template #cell-status="{ row }">
                <Badge :variant="(row as PriceGroupRecord).status === 'active' ? 'success' : 'neutral'">
                    {{ (row as PriceGroupRecord).status }}
                </Badge>
            </template>
            <template #cell-creator="{ row }">
                <TableAuditStamp :user="(row as PriceGroupRecord).creator" :at="(row as PriceGroupRecord).created_at" />
            </template>
            <template #cell-last_modifier="{ row }">
                <TableAuditStamp :user="(row as PriceGroupRecord).updater" :at="(row as PriceGroupRecord).updated_at" />
            </template>
            <template #actions="{ row }">
                <Link :href="route('admin.price-groups.edit', row.id)" class="text-sm text-accent hover:underline">{{ t('common.edit') }}</Link>
            </template>
        </DataTable>
        </div>
    </AppLayout>
